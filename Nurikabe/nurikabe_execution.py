...
def multi_puzzle(test_name: str) :
    input = pymzn.dzn2dict(f"./tests/multi/{test_name}.dzn")
    init : list = input["init"]
    cells_found = {}
    ...
    input["digits_encoding"] = 2
    return input

def single_puzzle(test_name: str) :
    input = pymzn.dzn2dict(f"./tests/single/{test_name}.dzn")
    input["digits_encoding"] = 1
    return input
                
def optimize_bound(input: dict):      
    bound = set()
    init : list = input["init"]
...
def default_bound(input: dict):
    max_value = max([max(row) for row in input["init"]])
    bound = 10 if max_value <= 9 else 100

    input["cells"] = set(range(0, bound))
    return input

def execute_model(input: dict):
    print("\nComputing the solution ... \n")
    start_time = time.time() 
    output = pymzn.minizinc("./nurikabe_python.mzn", data=input, output_mode="dict", output_vars=["solution"], two_pass=0)
    end_time = time.time() 
    ...
    try:
        for sol in output[0].values():
            for row in sol:
                for val in row:
                    pow_digit = pow(10, input["digits_encoding"] - 1)
